...
| [Sprint W1 — docs and the log](completed/sprint-w1-docs.md) | A stdlib generator whose Markdown subset was counted, not guessed. 40 pages, 0 broken links, and a fragment checker that found three dead anchors. |
| [Sprint W2 — numbers that are live](completed/sprint-w2-numbers.md) | The bench chart is drawn from each tag's baseline via `git show`; no history directory, no typed number. |
